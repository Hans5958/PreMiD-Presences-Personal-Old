		...
		*/
		...
		updateCallback.function = (): void => {
			...
			if (document.querySelector(".gameView")) {
				...
				const gameStatusLabel = document.querySelector(".gameStatusLabel").textContent
				...
				if (gameStatusLabel === "Waiting for more people...") {
					...
					const wpm = document.querySelector(".rankPanelWpm-self").textContent.toUpperCase()
		...
		*/
		...
		if (currentPath[0] === "pit") {
			...
			if (currentPath[1] === "profile") {
				...
				const option = document.querySelector("option[selected]").textContent.trim()
				const strong = document.querySelector("div.themeContent > div:nth-child(5) > strong").textContent.trim().slice(0, -1).split(" ")
