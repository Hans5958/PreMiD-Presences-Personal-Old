...
((): void => {
	...
	if (currentURL.hostname.startsWith("mania") || currentURL.hostname.startsWith("www")) {
		...
		if (currentPath[0] === "error") {
		...
		} else if (currentPath[0] === "tracksearch" || currentPath[0] === "mapsearch") {
			const searchSummary: string = document.querySelector("td.WindowText:nth-child(2)").textContent.trim().slice(8)
			presenceData.details = chooseTwo("Searching for a track", "Searching for a map")
			...
			if (currentPath[1] === "search") {
				const searchSummary = document.querySelector(".windowv2-textcontainer").textContent.trim().split(" ...")[0].slice(15)
				const usernameSearched = (document.querySelector("#UserUsername") as HTMLInputElement).value
				presenceData.details = "Searching for a user"
				if (usernameSearched) presenceData.state = `${usernameSearched}, ${searchSummary.slice(usernameSearched.length + 30)}`
				else presenceData.state = searchSummary[0].toUpperCase() + searchSummary.slice(1)
				...
				presenceData.state = document.querySelector(".WindowText .RowModCell_1:nth-of-type(2) a:nth-of-type(3)").textContent
			...
			if (currentPath[1] === "index" || currentPath[1] === undefined) presenceData.details = "Viewing thier private messages"
			else if (currentPath[1] === "compose") presenceData.details = "Writing a private message"
			else if (currentPath[1] === "reply") presenceData.details = "Replying a private message"
		...
		if (currentPath[0] === "error") {
		...
		} else if ((currentPath[0] === "" || currentPath[0] === "home") && currentPath.length === 1) {
			presenceData.details = "On the home page"
			...
			presenceData.state = document.querySelector(".panel-body dd:nth-of-type(2)").textContent.trim()
			...
			const details = []
			if ((document.querySelector("#username") as HTMLInputElement).textContent) details.push((document.querySelector("#username") as HTMLInputElement).textContent)
			if (document.querySelector("#s2id_mode").textContent) details.push(document.querySelector("#s2id_mode").textContent.slice(1))
			presenceData.details = "Searching for a user"
		...
		if (currentPath[0] === "auth") {
			...
			} else if (currentPath[1] === "register" || currentPath[1] === "resend_confirm") {
		...
		if (currentPath[0] === "") {
			...
			switch (currentPath[1]) {
				case "reference": presenceData.state = "API Reference"; break
				case "enum": presenceData.state = "Enumeration values"; break
				case "conventions": presenceData.state = "Conventions"; break
				case "changes": presenceData.state = "Changes"; break
			}
		}
